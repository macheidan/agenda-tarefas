...
- `fechamentos_mensais/{anoMes_marca}`, `dre_detalhes/{anoMes_marca}`, `vendas_itens/{anoMes_marca_cat_slug}`, `checkpoints/{id}` — categoria **Gestão** (Mesa do Dono / Dash / Vendas / DRE / Anotações), migrada do dashboard_pizzarias em 2026-08-25. Escrita só dos Apps Scripts das planilhas DRE e VENDAS LOJAS (dual-write nos 2 projetos Firebase) — exceto `checkpoints`, CRUD do admin pela aba Anotações. O Dash e o mês corrente da Mesa do Dono leem o JSON do coletor em fabiomachado.com.br/pizzas/data/ (CORS via .htaccess; token em `VITE_DASH_TOKEN`). O coletor é `scripts/dash/runner.py` (vendas de ontem e do mês no Saipos + IA/agenda/YouTube/newsletters), todo dia às 03:00 pela tarefa `DashboardColeta3h` — mudou para cá em 2026-09-02, saiu do machado-labs. **Ler `docs/gestao-migracao.md` antes de mexer.**
- `vendas_bairros/{anoMes_marca}` — sub-aba **Por Bairro** de Vendas: vendas por área de entrega, mês a mês (jan/2024 em diante). Fonte é o relatório `sales-by-store-district` do Saipos, o único lugar com essa quebra por período: a coleção `clientes` tem bairro mas só enxerga 90 dias e conta *cliente*, não pedido, e o backup do Saipos guarda o pedido sem o bairro. **Não é 1 doc por bairro**: são ~60 bairros × 32 meses × 2 lojas, e a tela precisa do histórico inteiro pra desenhar a linha mês a mês — um doc por bairro custaria ~2 mil leituras por abertura, e a quota do Firestore é o gargalo do projeto. Cada doc é um mês de uma loja: `{ marca, ano_mes, total_qtd, total_valor, bairros: [{b,q,v}] }` (~64 leituras pro histórico das duas). Grafias diferentes do mesmo lugar ("Passo D'Areia" × "Passo da Areia") viram uma linha só pela mesma normalização de `utils/relatoriosClientes.js` — sem isso o bairro se parte em dois e nenhuma das metades entra no top. Escrita só por `scripts/vendas/` (Playwright + Admin SDK): `coletar_bairros.py` → `importar_bairros.mjs`, pela tarefa mensal `vendas-bairros` (dia 2, 04:40, no manifesto `Hub/agenda/agenda.json`). **Roda só na madrugada**, como todo coletor de Saipos daqui: o Saipos aceita uma sessão por usuário, então com o PC da loja ligado o login daqui derruba a loja, a loja reconecta e derruba a gente — e o sintoma não é erro de login, é o app carregando pela metade (`mactrl.firebaseReady` falso) e a coleta morrendo em "campos de data não apareceram".
