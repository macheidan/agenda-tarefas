...
- `clientes/{loja}_{n}` — aba **Clientes**: base viva de quem comprou nos últimos 90 dias — nome, telefone, bairro, última compra, pedidos, valor total. Entra **todo mundo**, com telefone ou sem: mais da metade dos cadastros vem de marketplace (nome + CPF + endereço, telefone mascarado pelo iFood) e serve para faturamento/recência/bairro, não para campanha. Quem tem telefone com DDD do RS (51/53/54/55) é o público de WhatsApp — a tela separa nos botões *Com WhatsApp / Sem contato*. O coletor **religa** parte dos sem-telefone ao cadastro de balcão/site da mesma pessoa quando o CPF (ou nome+endereço) bate 1 para 1; CPF nunca sobe em claro, só o hash (`h`). **Não é 1 doc por cliente**: cada doc é um bloco de até 800 clientes em `itens: [{k,t,h,n,p,v,u,b,c,x,o,a,e}]` (+ `clientes/{loja}_meta`), senão a tela custaria milhares de leituras. Só leitura no cliente; escrita só por `scripts/clientes/` (Admin SDK + Playwright), todo dia às 04:10 (tarefa `ClientesColeta`)
